...
class Twitch:
    ...
    def twitch_receive_messages(self, amount=1024):
        ...
        if self.check_has_message(data):
            ret = [self.parse_message(line) for line in filter(None, data.split('\r\n'))];
            print "Messages", ret
            return ret


    """ Here be extension to send message """
    # def send_message(self, message, user):
    #     if len(message) > 0:
    #         # self.s.sendall(message + "\n")
    #         # self.s.sendto(1024, 'JOIN #%s\r\n' % user)
    #         # self.s.sendto('#%s\r\n' % user, message )
    #         print(message)

    def twitch_send_messages(self, message, channel):
        data = message

        # if not data: break
        # else:
        self.twitch_connect(self.user, self.oauth);
        self.s.send('PRIVMSG '+channel + data )
